<script>
    ...
    function getState(val)
    {
        if (val !== 'null')
        {
            $.ajax({
                type: "POST",
                url: '<?php echo base_url(); ?>bricks/customer_due/' + val,
...
<div class="right_col" role="main">
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                <div class="x_title">
                    <h2>Add New Transaction <small>Create New Customer Transaction</small></h2>
                ...
                <h3 style="color:green">
                    <?php
                    $msg = $this->session->userdata('save_customer_transaction');
                    if (isset($msg)) {
                        echo $msg;
                        $this->session->unset_userdata('save_customer_transaction');
                    }
                    ?>
                </h3>
                <div class="x_content">
                    <form action="<?php echo base_url(); ?>bricks/save_customer_transaction" method="POST" class="form-horizontal form-label-left" novalidate>
                        <div class="row">
                            <div class="col-md-7">
                                <div class="form-group">
                                    <label class="control-label col-md-3 col-sm-3 col-xs-12">Select Customer</label>
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <select name="customer_id" id="customer_id" onmouseout="getState(this.value);" class="form-control">
                                            <option value="null">Choose Customer</option>
                                            <?php foreach ($all_customer as $v_customer) { ?>
                                                <option value="<?php echo $v_customer->customer_id; ?>"><?php echo $v_customer->customer_name; ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
